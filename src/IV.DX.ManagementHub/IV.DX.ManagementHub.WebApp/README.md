...
- dataset view: таблица по метаданным — колонки из `QueryDefinition`,
  строки из запроса, сортировка, выбор строки (`features/dataset-view/`);
...
- действия: создание (POST, кнопка в тулбаре), редактирование главного элемента
  ...
  экспорт в `.dx`. Доступны и в строке таблицы, и в шапке превью — одним
  компонентом `unit-actions`. Строки коллекций добавляются, правятся во
