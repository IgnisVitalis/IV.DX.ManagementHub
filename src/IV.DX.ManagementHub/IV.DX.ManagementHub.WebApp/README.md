...
- dataset view: таблица по метаданным — колонки из `QueryDefinition`,
  строки из запроса, сортировка, множественный выбор чекбоксами
  (`features/dataset-view/`). Один выбранный элемент показывает превью,
  несколько — панель с массовыми действиями и списком выбранного;
